...
Mainly the loop does the following steps:
  ...
  主循环。循环关于卡、数据、时间戳和aba数据
  通过pstBufferData结构中的标志启用。
  这个循环可以直接调用，也可以从thread函数中调用。
  如果从线程函数调用它，它也将处理互斥对象和事件。
  该循环主要完成以下步骤:
  调用bWorkSetup(用户函数来设置所有细节)
  为缓冲区分配内存
  启动卡和传输取决于已设置的标志
  循环
  等待通知新数据或超时发生的中断
  读取缓冲区中的状态和当前位置
  重新计算位置和长度
  使用可用数据调用bWorkDo(用户函数)
  再次设置已处理的数据对card可用
  直到(中止或错误)
  调用vWorkClose(关闭和清理文件的用户函数)
  清理缓冲区
...
void vMainLoop (
    ...
    SPCM_WORK_DO*       pfn_bWorkDo,            // callback function that is called if new data is available（新信号触发）
    SPCM_WORK_CLOSE*    pfn_vWorkClose,         // callback function（回调函数） at the end of run
    SPCM_ABORTCHECK*    pfn_bAbortCheck,        // callback function for abort checking on each update loop
    ST_THREADDATA*      pstThreadData)          // threaddata for synchronization（同步）. Can be NULL if non threaded version is used
...
// ***********************************************************************
    ...
    // 主循环this is the main loop that waits for data. As we defined a timeout this routine is latest called in our timeout interval
    while (!dwError)
        {
        ...
        if (lStatus & M2STAT_DATA_END)
            {
            // DATA_END can also occur if all data has been transfered from card after overrun（溢出）
        ...
        // read out the timestanp positions and recalculate it to avoid rollover时间戳
        if (pstBufferData->bStartTimestamp)
            {
...
// ***********************************************************************
	...
	//我们将私有缓冲区数据设置为给定缓冲区的副本，并为单独的传输禁用开始标志
    for (i=0; i<3; i++)
        {
	...
	//开始
    for (i=0; (i<nThreadCount) && bOk; i++)//start ? threads
    ...
    for (i=0; (i<nThreadCount) && bOk; i++)
        if (!stThreadData[i].pstBufferData->bStartData)//stThreadData[i].pstBufferData->bStartData==0
            {
    ...
    for (i=0; (i<nThreadCount) && bOk; i++)
        if (stThreadData[i].pstBufferData->bStartData)//stThreadData[i].pstBufferData->bStartData==1
            {
            stThreadData[i].dwEventFlags |= EVENTFLAG_START;
            spcm_vSignalEvent (&stThreadData[i].hEventStart);
    ...
    while (bOk)
        ...
        // on each update we can abort the loop中止
        if (pfn_bAbortCheck)
            if (pfn_bAbortCheck (pvWorkData, &stBufferData[0]))
                {
                spcm_dwSetParam_i32 (pstBufferData->pstCard->hDrv, SPC_M2CMD, M2CMD_CARD_STOP | M2CMD_DATA_STOPDMA | M2CMD_EXTRA_STOPDMA);//中止语句
                printf ("\n...Aborted by user\n");
                break;
                }
